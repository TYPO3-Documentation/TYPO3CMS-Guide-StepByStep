# Add a Checkbox Field to a Page Record

<!-- #TYPO3v13 #Beginner #Backend #Configuration #SitePackage @dragos_tuluc -->

Page properties are the backend form for records in TYPO3’s `pages` table. Adding a checkbox field there is a practical next step into [TCA](https://docs.typo3.org/permalink/t3tca:start), because you extend a TYPO3 Core record and get a simple on/off field in the backend.

## Learning objective

In this step-by-step guide you will add one custom checkbox field to the TYPO3 `pages` record and make it editable in page properties.
...
>
> Replace `tx_my_site_package_highlight_page` with your own custom field name. Custom fields on Core tables usually start with the prefix `tx_`.
...
2. Add the following SQL definition:

```sql
CREATE TABLE pages (
    tx_my_site_package_highlight_page tinyint(4) unsigned DEFAULT '0' NOT NULL
);
```
...
ExtensionManagementUtility::addTCAcolumns(
    'pages',
    [
        'tx_my_site_package_highlight_page' => [
            'exclude' => true,
            'label' => 'Highlight this page',
            'config' => [
                'type' => 'check',
                'items' => [
                    [
...
> [!TIP]
> A checkbox field is useful for simple yes/no decisions, such as marking a page as highlighted, featured, or hidden from a custom output.
...
4. In **Analyze Database Structure**, apply the suggested change for `tx_my_site_package_highlight_page`.
5. Open the page properties of any page.
6. Verify that the new field **Highlight this page** appears below the **Page title** field.
7. Select the checkbox and save the page.

After saving, the checkbox state should remain stored in the page record.

## Summary

Congratulations! You added one custom checkbox field to a TYPO3 page record and made it editable in page properties.
...
* [Checkbox fields in TCA](https://docs.typo3.org/m/typo3/reference-tca/main/en-us/ColumnsConfig/Type/Check/Index.html)
